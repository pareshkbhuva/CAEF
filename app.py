...
# Public demo rate limit - per-IP, per-day (abuse prevention, no login needed)
...
def public_analyze():
    ...
    # Strip internal scores from public response
    public_res = {
...
def api_ask():
    ...
    t0 = time.time()
    # FIX: handler uses "generate_and_score", not "ask"
    res, err = call_runpod({"action": "generate_and_score", "question": d["question"],
